...
def rule_cve202430394(configuration, commands, device, devices):
    """
    This rule checks for CVE-2024-30394 vulnerability in Juniper Networks Junos OS and Junos OS Evolved.
    The vulnerability allows an unauthenticated, network-based attacker to cause a Denial of Service (DoS)
    by sending specific EVPN type-5 routes that trigger a stack-based buffer overflow in rpd.
    ...
    assert recent_crashes == 0, (
        f"Device {device.name} is vulnerable to CVE-2024-30394. "
        f"The device is running a vulnerable version with EVPN enabled and has {recent_crashes} "
        "recent rpd crashes. This can indicate exploitation through malformed EVPN type-5 routes. "
        "Please upgrade to one of the following fixed versions: "
        "Junos OS: 21.2R3-S7, 21.4R3-S5, 22.1R3-S4, 22.2R3-S2, 22.3R3-S1, 22.4R3, 23.2R2, 23.4R1 or later; "
        "Junos OS Evolved: corresponding EVO versions. "
        "For more information, see https://supportportal.juniper.net/JSA79094"
    )
