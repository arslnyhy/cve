...
def rule_cve202439515(configuration, commands, device, devices):
    """
    This rule checks for CVE-2024-39515 vulnerability in Juniper Networks Junos OS and Junos OS Evolved.
    The vulnerability allows an unauthenticated network-based attacker to cause a Denial of Service (DoS)
    by sending malformed BGP packets that cause rpd to crash when BGP traceoptions are enabled.
    ...
    assert not recent_crashes, (
        f"Device {device.name} is vulnerable to CVE-2024-39515. "
        ...
        "For more information, see https://supportportal.juniper.net/JSA88099"
    )
