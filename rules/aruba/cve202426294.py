...
@high(
    name='rule_cve202426294',
    platform=['aruba_os'],
    commands=dict(
        show_version='show version',
        show_web_config='show configuration | include web-management'
    ),
)
def rule_cve202426294(configuration, commands, device, devices):
    """
    This rule checks for CVE-2024-26294 vulnerability in Aruba ClearPass Policy Manager.
    ...
    web_config = commands.show_web_config
    web_enabled = 'web-management' in web_config

    # Assert that the device is not vulnerable
    assert not web_enabled, (
        f"Device {device.name} is vulnerable to CVE-2024-26294. "
