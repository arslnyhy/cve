...
def rule_cve202442502(configuration, commands, device, devices):
    """
    This rule checks for CVE-2024-42502 vulnerability in ArubaOS devices.
    The vulnerability allows authenticated attackers to execute arbitrary commands through
    the command line interface, which could lead to complete system compromise through
    command injection.
    """
    ...
    assert not cli_enabled, (
        f"Device {device.name} is vulnerable to CVE-2024-42502. "
        ...
        "that could lead to arbitrary command execution and system compromise."
    )
