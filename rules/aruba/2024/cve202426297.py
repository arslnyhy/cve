...
def rule_cve202426297(configuration, commands, device, devices):
    """
    This rule checks for CVE-2024-26297 vulnerability in Aruba ClearPass Policy Manager.
    ...
    web_server = commands.show_web_server
    web_enabled = 'web-server enabled' in web_server

    # Assert that the device is not vulnerable
    assert not web_enabled, (
        f"Device {device.name} is vulnerable to CVE-2024-26297. "
        "The device is running a vulnerable version with web server enabled, "
