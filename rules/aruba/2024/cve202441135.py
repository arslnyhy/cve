...
def rule_cve202441135(configuration, commands, device, devices):
    """
    This rule checks for CVE-2024-41135 vulnerability in Aruba EdgeConnect SD-WAN devices.
    ...
    assert not cli_enabled, (
        f"Device {device.name} is vulnerable to CVE-2024-41135. "
        "The device is running a vulnerable version with CLI access enabled, "
        "which makes it susceptible to authenticated remote code execution attacks "
