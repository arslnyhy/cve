...
@high(
    ...
    commands=dict(
        ...
        show_confd_conf='show running-config | include confd.conf'
    ),
)
def rule_cve202420381(configuration, commands, device, devices):
    ...
    if '<webui>' in confd_conf_output and '<enabled>true</enabled>' in confd_conf_output:
        ...
        assert False, (
            ...
            "The device is running a vulnerable version AND has the 'webui' feature enabled in the confd.conf configuration, "
